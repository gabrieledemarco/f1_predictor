...
Crontab (venerdi 18:00 ogni GP weekend):
    0 18 * * 5 cd /path/to/f1_predictor && python scripts/collect_odds.py
...
Output:
    ...
    Ogni riga e' un JSON record con: driver, market, odds, timestamp.
...
Nota Layer 4:
    ...
    Verifica: python train_pipeline.py --list-versions
    (colonna 'Calib' passa da 'NO' a 'OK')
"""
...
def load_api_key() -> str:
    """Carica THE_ODDS_API_KEY da .env o variabile d'ambiente."""
    # Tenta .env nella root del progetto
...
def fetch_odds(api_key: str, market: str = "outrights") -> list[dict]:
    """
    Scarica le quote F1 da The Odds API.

    Args:
        api_key: Chiave API The Odds API.
        market: 'outrights' (vincitore gara/campionato) o 'h2h' (testa-a-testa).

    Returns:
        Lista di record odds normalizzati.
    """
    import requests
    ...
    params = {
        ...
        "markets":      market,
        "oddsFormat":   "decimal",
        "bookmakers":   "pinnacle",   # Pinnacle = closing odds + bassa vig
    }
...
def save_records(records: list[dict], out_dir: Path) -> Path:
    """Salva i record in un file JSONL con timestamp nel nome."""
...
def main():
    ...
    parser.add_argument(
        ...
        help="Mercato da scaricare (default: outrights)"
    )
    parser.add_argument(
        "--out-dir",
        default="data/pinnacle_odds",
        help="Directory output JSONL (default: data/pinnacle_odds)"
    )
    parser.add_argument(
        "--all-markets",
        action="store_true",
    ...
    if total_existing < 100:
        ...
        print(f"  Il calibratore isotonic si attivera' automaticamente al raggiungimento.")
    else:
